...
import { getMesasRequest, updateMesasRequest, borrarMesasRequest } from "../api/mesas.api";
...
function MesasManager() {
  ...
  const borrarMesa = async (id) => {
    try {
      const resp = await borrarMesasRequest(id);
      setMesas(mesas.filter((mesas) => mesas.idMesa !== id));
    } catch (error) {
      console.error(error);
    }
  };
          ...
          {mesas.map((mesa) => (
            ...
            >
              ...
              <p className="text-white">
                {mesa.reservadoMesas == 1 ? "Reservada" : "Libre"}
              </p>
              <div className="flex-col flex gap-1 mt-5 justify-center h-10">
                <div>
                  <button
                    className="bg-amber-500 px-2 py-1 mr-1 text-white rounded-md"
                    onClick={() => {
